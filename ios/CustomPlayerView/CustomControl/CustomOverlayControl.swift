...
fileprivate struct TitleConstants {
  ...
  static var font: Double = 13
  static var yOffset: Double = 10
}
...
//  }
  ...
  private func addTitleLandscape() {
    ...
    let frame = CGRect(x: videoRect.size.width/2 - TitleConstants.width/2 , y: videoRect.origin.y + TitleConstants.yOffset, width: TitleConstants.width, height: TitleConstants.height)
    titleLabel.textAlignment = .center
    titleLabel.frame = frame
    self.addSubview(titleLabel)
  }
  private func addTitlePortrait() {
    guard let videoRect = self.session?.playerLayer.videoRect else { return }
    let frame = CGRect(x: videoRect.size.width - TitleConstants.width - TitleConstants.yOffset , y: videoRect.origin.y, width: TitleConstants.width, height: TitleConstants.height)
